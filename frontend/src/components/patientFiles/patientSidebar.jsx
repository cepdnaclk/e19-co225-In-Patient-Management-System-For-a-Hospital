...
function PatientSidebar(props) {
    ...
    const Menus = [
        ...
        { title: "Log out", src: <LuLogOut size={30} />, path: "" },
    ];
            ...
            <div className={` ${open ? "w-72" : "w-20 "} bg-gradient-to-r from-black to-black h-screen p-5 pt-8 duration-300 fixed text-white`}>
                ...
                <ul className="pt-6">
                    {Menus.map((Menu, index) => (
                        <li key={index} onClick={(e, path) => navigateDashBoard(e, Menu.path)} className={`flex rounded-md p-2 cursor-pointer hover:bg-blue-500 text-gray-300 text-sm items-center gap-x-4 ${Menu.gap ? "mt-9" : "mt-2"}  `}>
                            <span>{Menu.src}</span>
